...
import { isDeletePermission, isResourcePermission, PolicyDecision } from "@backstage/plugin-permission-common";
...
export async function cannotDeleteEntities(opaClient: OpaClient) {
  return async (request: PolicyQuery): Promise<PolicyDecision> => {
    ...
    if (isResourcePermission(request.permission, "catalog-entity") && isDeletePermission(request.permission)) {
