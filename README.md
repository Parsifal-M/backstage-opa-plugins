...
> **Note:** `opa-authz-react` works under both the legacy and new Backstage frontend systems as-is. `RequireOpaAuthz` and its hooks are plain components (like `RequirePermission` from `@backstage/plugin-permission-react`), not extension-point occupants, so there's nothing NFS-specific to build for them. The API they depend on is registered for both systems: `src/index.ts` (legacy) and `src/alpha.tsx` via `ApiBlueprint` (new frontend system).
