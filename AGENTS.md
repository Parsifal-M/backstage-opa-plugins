...
| `plugins/backstage-plugin-opa-authz-react`              | `@parsifal-m/backstage-plugin-opa-authz-react`              | Web library (`web-library`)                     | React components (`RequireOpaAuthz`) and hooks (`useOpaAuthz`, `useOpaAuthzManual`) for hiding/showing UI elements based on OPA decisions. Works unmodified under both the legacy and new frontend systems — it's a plain component consumed directly by other plugins' JSX (like `@backstage/plugin-permission-react`'s `RequirePermission`), not an extension-point occupant, so there's nothing to migrate. The API it depends on is registered for both systems: `src/index.ts` (legacy) and `src/alpha.tsx` via `ApiBlueprint` (new frontend system). |
...
- `backstage-plugin-opa-authz-react` works under both the legacy and new frontend systems as-is. `RequireOpaAuthz`/`useOpaAuthz`/`useOpaAuthzManual` are plain components/hooks (like `RequirePermission` from `@backstage/plugin-permission-react`), not extension-point occupants, so there is no NFS-specific version to build for them. The `opaAuthzBackendApiRef` API they depend on is registered for both systems: `src/index.ts` (legacy) and `src/alpha.tsx` via `ApiBlueprint` (new frontend system).
